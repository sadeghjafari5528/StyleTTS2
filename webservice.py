...
config = yaml.safe_load(open("StyleTTS2-LibriTTS/Models/LibriTTS/config.yml"))
...
if not os.path.exists(model_path):
    ...
    # Move or copy the file to the expected path if needed
    src_path = os.path.join(os.path.dirname(model_path), "Models", "LibriTTS", "epochs_2nd_00020.pth")
    if src_path != model_path and os.path.exists(src_path):
...
def synthesize():
    ...
    try:
        ref_s = compute_style(tmp_audio_path)
        wav = inference(text, ref_s, alpha=0.3, beta=0.7, diffusion_steps=5, embedding_scale=1)
        # Save output to a BytesIO buffer
        buf = io.BytesIO()
        torchaudio.save(buf, torch.from_numpy(wav).unsqueeze(0), 24000, format='wav')
        buf.seek(0)
        return send_file(
            buf,
            mimetype='audio/wav',
